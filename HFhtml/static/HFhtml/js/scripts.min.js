...
$(document).ready(function () {
    ...
    $('.popup-with-zoom-anim').click(function () {
        ...
        if ($(this).text() === 'Получить консультацию' || $(this).text() === 'Заказать звонок' || $(this).attr('title') === 'Обратный звонок') {
            $('#txt').val('Обратный звонок');
        }
    });

    $('.drop-down-header').click(function () {
        $('.am-tooltip').toggle('display:block');
        $('.js-region').val($('.drop-down-header').text());
    });

    $('.js-region').change(function () {
        $('#city').val($(".js-region option:selected").val());
    });

    $('.buy').click(function (e) {
        e.preventDefault();
        var basketcounter = parseInt($('#basketbtn span').text());
        if ( $('.shopping-cart').is('product') ){
            var view = $('.shopping-cart .product');
        } else {
            var view = $('.shopping-cart .column-labels');
        }

        var data = {}
        var crsf_token = $(this).parent().children('input').val();
        data['csrfmiddlewaretoken'] = crsf_token;
        data['name'] = $(this).data('name');
        data['price'] = parseInt($(this).data('price'));
...
window.onscroll = function () {
    ...
    if (scrolled > 100) {
        document.getElementById('basketbtn').style.display = 'block';
